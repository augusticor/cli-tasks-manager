...
class TaskManager {
  ...
  printStylizedTasks() {
    ...
    for (let i = 0; i < taskListOnArrayMode.length; i++) {
      ...
      isAchievedYet =
        eachTask.achievedIn !== null ? colorizeText('Complete', 'green') : colorizeText('Incomplete', 'red');
  ...
  printArrayOfTasks(arrayOfTasks) {
    ...
    arrayOfTasks.forEach((task, i) => {
      ...
      const taskStatus = achievedIn
        ? colorizeText(`Completed on ${this.getDateWithFormat(task.achievedIn)}`, 'green')
        : colorizeText('Incomplete', 'red');
